...
def EditTrainers(data_dir, model_dir, rep_dir, numsplits):
  tnames = ['train_CD_RNA1seq_layer1.pbtxt',
            'train_CD_RNA2seq_layer1.pbtxt',
            'train_CD_joint_layer.pbtxt',
            'train_CD_joint_layer2.pbtxt',
			'train_CD_joint_layer2c.pbtxt']
  for tname in tnames:
  ...
  t_op_file = os.path.join('trainers', 'classifiers', 'baseclassifier_1.pbtxt')
  t_op = util.ReadOperation(t_op_file)
  for i in range(1, 2):
  ...
  t_op_file = os.path.join('trainers', 'classifiers', 'baseclassifier_1c.pbtxt')
  t_op = util.ReadOperation(t_op_file)
  for i in range(1, 2):
    t_op_file = os.path.join('trainers', 'classifiers', 'split_%dc.pbtxt' % i)
    t_op.data_proto_prefix = rep_dir
    t_op.data_proto = os.path.join('split_%d' % i, 'datac.pbtxt')
    t_op.checkpoint_prefix = model_dir
    t_op.checkpoint_directory = os.path.join('classifiers','split_%d' % i)  
    with open(t_op_file, 'w') as f:
      text_format.PrintMessage(t_op, f)
  ...
  t_op = util.ReadOperation(t_op_file)
  for i in range(1, 2):
    t_op_file = os.path.join('trainers', 'classifiers', 'split.pbtxt')
    t_op.data_proto_prefix = rep_dir
    t_op.data_proto = os.path.join('split_%d' % i, 'data.pbtxt')
    t_op.checkpoint_prefix = model_dir
    t_op.checkpoint_directory = os.path.join('classifiers','split_%d' % i)  
  ...
  t_op_file = os.path.join('trainers', 'classifiers', 'baseclassifierc.pbtxt')
  t_op = util.ReadOperation(t_op_file)
  for i in range(1, 2):
    t_op_file = os.path.join('trainers', 'classifiers', 'splitc.pbtxt')
    t_op.data_proto_prefix = rep_dir
    t_op.data_proto = os.path.join('split_%d' % i, 'datac.pbtxt')
    t_op.checkpoint_prefix = model_dir
    t_op.checkpoint_directory = os.path.join('classifiers','split_%d' % i)  
